...
export class DataDragonService {
  ...
  // singleton pattern to ensure version is loaded before any data requests
  ...
  async getVersion(): Promise<string | null> {
    if (!this.version) await this.ensureVersion();
    return this.version;
  }

  async getAllChampions(): Promise<any[]> {
    if (!this.baseUrl) await this.ensureVersion();
